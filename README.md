...
- **lite** — облегчённый набор без GSD-machinery:
  - плагины — только `superpowers`, `context-mode`, `context7` (без `gsd`);
...
- фильтрует записи хуков и ключ `statusLine` в `settings.json` под новый вариант: переход в lite
  снимает GSD-хуки и, если она стояла, запись `gsd-context-meter.mjs` из `statusLine` (при этом
  твой собственный, не-бандловый `statusLine` не трогается).
...
- **(m) объединить (merge)** — дефолт.
  - **любой `.json`** (твой добавленный, `settings.json`, `setting-templates/*.json`) —
    ...
    если путь до `$HOME` содержит пробел). Это поведение **full**-варианта: **lite** не ставит
    `gsd-context-meter.mjs` вовсе и `statusLine` не трогает (при переключении full→lite прежняя
    запись, если она там стояла, снимается — см. «Варианты бандла» выше).
...
- **gsd-context-meter.mjs** (`statusLine.command`, регистрация — см. «Как работает установщик»
  выше; **только full-вариант** — lite не ставит этот файл и не трогает `statusLine`, см.
  «Варианты бандла»). Оборачивает собственный `gsd-statusline.js` gsd-core: зовёт его как чёрный ящик и
