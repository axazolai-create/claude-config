...
- **чинит `claude_md_assembly.mode: "link"`, если рут его не импортирует** — при этом режиме
  gsd-core пишет сгенерированный контекст проекта + профиль (`/gsd-profile-user`) в отдельный
  `.claude/CLAUDE.md`, но сам не гарантирует, что рутовый `CLAUDE.md` реально его `@`-импортирует.
  На практике это давало 18-килобайтный сгенерированный файл, который вообще никогда не грузился
  ни в одну сессию. **Перепроверяется каждую сессию** (не одноразово — `.claude/CLAUDE.md` может
  появиться позже первой сессии, например от `/gsd-profile-user`), само-останавливается, как
  только импорт появился. Пропускает: рут-файл отсутствует, уже импортирует
  `.claude/CLAUDE.md`, или сам похож на сгенерированный GSD. Тумблер: `CLAUDE_GSD_LINK_IMPORT=0`.
...
CLAUDE_GSD_LINK_IMPORT=0         # не чинить отсутствующий @.claude/CLAUDE.md импорт
```
