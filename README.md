...
- **leanmode-subagent.mjs** (`SubagentStart`, событие используется впервые в этом репо — до сих
  ...
  `additionalContext` — оставлено в коде на случай, если будущая версия харнесса начнёт это
  рендерить, но эмпирически (2026-07-11, три реальных запуска саб-агента) подтверждено, что
  `systemMessage` от `SubagentStart` сейчас нигде не отображается: ни в баннере (который
  харнесс рисует до отработки хуков и переписать нельзя в принципе), ни отдельной строкой. Тот
  же уровень виден на практике через отдельный механизм — см. `session-init.mjs` выше.
