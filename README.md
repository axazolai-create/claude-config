...
- **проверяет снапшот stack-rules** (только подсказка, файлы не трогает) — сверяет фронтматтер
  `.claude/stack-rules.md` с текущим состоянием `~/.claude/rules-src/` и стеком проекта
  (`hooks/lib/stack-rules-check.mjs`); если снапшота нет или он устарел, дописывает в
  `additionalContext` указание собрать его суб-агентом; когда всё совпадает — молчит.
  Подробности механизма — раздел «Правила стека (stack-rules)» ниже. Тумблер:
...
CLAUDE_STACK_RULES=0             # не проверять снапшот stack-rules (см. раздел ниже)
```
...
- **Пер-проектный снапшот `<проект>/.claude/stack-rules.md`** — скомпилированная выжимка из
  ...
  `~/.claude/rules-src/README.md` § «Building stack-rules». `paths:`-фронтматтер в исходниках
  сохранён — теперь это метаданные отбора для компилятора, Claude Code его не читает.
...
- **Проверка на старте сессии** — шаг `session-init.mjs` + `hooks/lib/stack-rules-check.mjs`:
  сверяется фронтматтер снапшота — `sourceHash` (хеш путь+размер+mtime всех файлов
  `~/.claude/rules-src/`) и `stackFingerprint` (хеш набора обнаруженных сигнатурных файлов
  стека: package.json, next.config.*, pyproject.toml и т.д.). Снапшота нет или хеши
  разошлись → в `additionalContext` дописывается указание диспатчить суб-агента на
  (пере)сборку (хуки сами суб-агентов запускать не могут, поэтому хук только инструктирует);
  всё совпадает → молчит. Проверить руками:
  `node ~/.claude/hooks/lib/stack-rules-check.mjs [корень-проекта]`. Тумблер:
  `CLAUDE_STACK_RULES=0`.
