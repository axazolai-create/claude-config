...
- **leanmode-subagent.mjs** (`SubagentStart`, событие используется впервые в этом репо — до сих
  ...
  ни отдельной строкой там. Независимо подтверждено первоисточником: у стороннего плагина
  `ponytail`, с которого списан дизайн (см. выше), `ponytail-runtime.js` для нативного Claude
  Code на `SubagentStart` тоже эмитит только `additionalContext`, без `systemMessage` — его
  автор явно не полагается на это поле там же. Тот же уровень виден оркестратору на практике
  через отдельный механизм — см. `session-init.mjs` выше. Отдельное, ещё не переподтверждённое
...
идемпотентны, см. «Авто-инициализация проектов» выше; раньше были однократными — оказалось багом
таймингов, когда файл появлялся позже первой сессии.)
