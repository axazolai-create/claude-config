...
- **statusline.mjs** (`statusLine.command`, регистрация — см. «Как работает установщик» выше;
  **все три профиля** — `full`, `base`, `lite`, единый рендерер вместо прежнего выбора между
  ...
  3. **контекст** — токены и процент, например `165.6K/1M 17%`, раскрашенные и помеченные по
     двум разным шкалам: **цвет** — по проценту окна модели (15/45/70/85/95% → серый/зелёный/
     ...
     модели (`~/.claude/state/autocompact.json`) → окно целиком. Дефолт — именно полное окно,
     а не угаданный резерв;
  ...
  Сегмент 6 выбирает, что назвать, детерминированно, в этом порядке: фронтматтер `current` из
  `.ultrapowers/ROADMAP.md`; иначе — единственная фаза со `status: running` (0 или несколько
  совпадений — сегмент молчит, а не гадает); иначе — самый свежий SDD-ledger под
  `.ultrapowers/sdd/`. Раньше выбор шёл по mtime файла, и чекаут менял то, что показывает бар;
  теперь mtime — только тай-брейкер среди ledger'ов и никогда не перевешивает объявленную фазу.
  Фаза печатает счёт задач и **никогда** процент — `08 (2/6) running`, или `08 planned`, если у
  фазы ещё нет плана: фаза со снятой (`tasks_dropped`) задачей формулирует счёт в полях, а
  причину — прозой, и вычисленный процент занижал бы уже завершённую фазу.
