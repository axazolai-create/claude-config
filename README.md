...
- **lite** — облегчённый набор без GSD-machinery:
  ...
  - ровно 6 хуков: `secrets-gate`, `deny-curated-claude-md`, `graphify-global-sync`,
    `leanmode-subagent`, `token-usage-log`, `session-init` (последний работает, но пропускает все
...
- фильтрует записи хуков и ключ `statusLine` в `settings.json` под новый вариант: переход в
  base/lite снимает GSD-хуки и, если она стояла, подменяет запись `gsd-context-meter.mjs` в
  `statusLine` на собственный рендерер этих профилей — `statusline.mjs`; обратный переход
  возвращает `gsd-context-meter.mjs` (при этом твой собственный, не-бандловый `statusLine`
  не трогается).
...
- **(m) объединить (merge)** — дефолт.
  - **любой `.json`** (твой добавленный, `settings.json`, `setting-templates/*.json`) —
    ...
    если путь до `$HOME` содержит пробел). Это поведение **full**-варианта: **base/lite** не
    ставят `gsd-context-meter.mjs` вовсе и регистрируют вместо него
    `node "<путь>/statusline.mjs"` — рендерер, который рисует всю строку сам. Своей бандл
    считает запись, указывающую на любой из этих двух скриптов, поэтому переключение профиля
    меняет её в обе стороны и не оставляет `statusLine`, целящийся в только что удалённый файл
    (см. «Варианты бандла» выше).
...
- **gsd-context-meter.mjs** (`statusLine.command`, регистрация — см. «Как работает установщик»
  выше; **только full-вариант** — base/lite не ставят этот файл и регистрируют `statusline.mjs`,
  см. «Варианты бандла»). Оборачивает собственный `gsd-statusline.js` gsd-core: зовёт его как чёрный ящик и
...
- **statusline.mjs** (`statusLine.command`, **base/lite**). Оборачивать нечего — gsd-core в этих
  профилях нет, — поэтому строка рисуется целиком: доступные обновления компонентов, счётчик
  контекста (`[420k/1000k] 42%`) и состояние проекта (GSD `.planning/`, план `.ultrapowers/sdd/`
  или просто папка+ветка git). То же ключевое свойство: любая ошибка даёт пустой вывод и код 0,
  statusline не ломается никогда.
