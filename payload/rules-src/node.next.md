...
- Git worktrees + pnpm store: keep pnpm's default per-project store inside the project — do NOT
  relocate it out of the worktree (`enableGlobalVirtualStore` / a moved `virtual-store-dir`). An
  out-of-tree store breaks phantom-dependency resolution: an UNDECLARED import a package finds via
  Node's upward `node_modules` walk (e.g. `@hookform/resolvers`'s `/zod` subpath, with `zod` in
  neither its deps nor peerDeps) stops resolving, and `turbopack.root` does NOT fix it — it scopes
  what Turbopack may READ, not Node module resolution.
