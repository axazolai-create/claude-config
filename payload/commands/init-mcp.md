---
description: Set up / switch / remove per-project MCP servers (git host, Postgres, Neo4j, web search) with consent, auth, and verification
argument-hint: "[github|gitlab|postgres|neo4j|search|status]"
...
- Show me a short summary: "Current MCP: <list>. Git remote: <host>." Then offer the actions
  below. If I passed an argument (`github`/`gitlab`/`postgres`/`neo4j`/`search`/`status`),
  jump to it; `status` just prints this summary and stops.
